...
\\
Finalement, comme la gestion des erreurs se fait via les exceptions et que notre cas typique est de lever une exception lorsqu'une opération avec continuation échoue, nous avons définit une macro permettant de construire simplement la gestion du cas. Comme c'est à chaque fois de construire la fonction qui lance avec un message construit dynamiquement basé sur le token, une macro simple qui prend un ensemble d'éléments à concaténer pour avoir le message final est assez avantageuse par rapport à l'emploi d'une fonction simple: On ne veut évaluer le message qui si l'opération a échouée et une macro nous évite d'avoir à écrire un nouveau lambda à chaque fois.
